...
function getCollapseTitleState(productName, versionName) {
   return localStorage['group_' + _.escape(productName) + '_' + _.escape(versionName)] ? "" : "collapsed";
}

function getCollapseState(productName, versionName) {
    return localStorage['group_' + _.escape(productName) + '_' + _.escape(versionName)] ? "collapse in" : "collapse";
}
...
function displayTable(productAndVersionBooks) {
    ...
    var matchingBookCount = _.reduce(productsAndVersions, function(memo, element) {
         ...
         */
        bookList.on('shown.bs.collapse', function() { localStorage['group_' + _.escape(productName) + '_' + _.escape(versionName)] = true });
        bookList.on('hidden.bs.collapse', function() { localStorage['group_' + _.escape(productName) + '_' + _.escape(versionName)] = false });
