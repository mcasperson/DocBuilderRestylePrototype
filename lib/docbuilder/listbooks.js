    ...
    _.each(productsAndVersions, function(element, index, list) {
        ...
        _.each(productAndVersionBooks, function(bookElement) {
            var bookListing = jQuery(
                '<div>\
                    <h2 class="bookTitle">' + _.escape(bookElement.title) + '</h2> \
                    <div class="btn-group"> \
                        <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown"> \
                            <span class="fa fa-angle-down"></span> \
                        </button> \
                        <ul class="dropdown-menu" role="menu"> \
                            <li><a href="' + SPEC_EDIT + element.id + '">Edit</a></li> \
                            <li><a href="' + element.id + '/remarks">Remarks</a></li> \
                            <li><a href="' + element.id + '/build.log">Build Log</a></li> \
                            <li><a href="' + element.id + '/publican.log">Publican Log</a></li> \
                            </li>' + (bookElement.buildSuccess?'<span class="pficon pficon-ok"></span>':'<span class="pficon-layered"><span class="pficon pficon-error-octagon"></span><span class="pficon pficon-error-exclamation"></span></span></li>') + '\
                        </ul> \
                    </div> \
                </div>');
